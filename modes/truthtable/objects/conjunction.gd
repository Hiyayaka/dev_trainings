...
const OPERATOR_LIST:Array=["^","&","and"]

func build_formula() -> bool:
	if !next:
		expression.error="Conjunction missing right hand side object."
		return false
	if next is Identifier or next is OExpression or next is Negation:
		pass
	else:
		expression.error="Conjuction right hand side object not Identifier or Expression."
		return false
	if !prev:
		expression.error="Conjunction missing left hand side object."
		return false
	if name.length() == 1:
...
func build_values() -> void:
	for i in range(prev.values.size()):
		if bool(prev.values[i]) == true and bool(next.values[i]) == true:
			values.append(1)
		else:
			values.append(0)
	expression.rem(prev)
